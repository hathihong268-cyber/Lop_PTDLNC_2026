"""
Module Advanced RAG cho Buổi 09: Snapshot Baseline từ Buổi 08.
Đóng vai trò baseline để so sánh với Multi-query Retrieval và Parent-Child Retrieval Pipeline của Buổi 09.
...
# Import loader, validator, collection naming, Gemini và Chroma helpers từ baseline Buổi 09
from rag import (
    load_chunks,
    validate_chunk,
    validate_embeddings,
...
def load_advanced_config() -> dict[str, Any]:
    """
    Nạp và kiểm tra toàn diện cấu hình Advanced RAG từ file .env cục bộ của Buổi 09.
...
def query_advanced_rag(
    ...
    """
    Quy trình Hỏi-Đáp RAG Nâng cao hoàn chỉnh (Step 08 baseline):
...
def main():
    parser = argparse.ArgumentParser(description="Advanced RAG Buổi 09 - Baseline Loader, Validator, Status, Indexing & Query")
    ...
    prepare_parser = subparsers.add_parser("prepare-semantic", help="Nạp embeddings thật vào ChromaDB Buổi 09")
