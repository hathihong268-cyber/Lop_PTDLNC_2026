...
def _build_page_offset_map(document: DocumentText) -> tuple[str, list[tuple[int, int, int]]]:
    """Xây dựng full-text và bảng ánh xạ offset → số trang.

    Trả về (full_text, [(page_number, char_start, char_end), ...]).
    Dùng chung cho fixed_chunks và semantic_chunks để tính page_start/page_end chính xác.
    """
    parts: list[str] = []
    page_map: list[tuple[int, int, int]] = []
    cursor = 0
    for page in document.pages:
        page_start_offset = cursor
        parts.append(page.text)
        cursor += len(page.text)
        if page is not document.pages[-1]:  # dấu phân cách "\n\n" giữa các trang
            cursor += 2
        page_map.append((page.page, page_start_offset, cursor))
    return "\n\n".join(parts), page_map


def _page_at(offset: int, page_map: list[tuple[int, int, int]], fallback: int) -> int:
    """Trả về số trang chứa vị trí `offset` trong full_text."""
...
def fixed_chunks(document: DocumentText, size: int, overlap: int) -> list[dict[str, Any]]:
    """Cắt fixed-size với overlap; tính page_start/page_end theo offset chính xác."""
    full, page_map = _build_page_offset_map(document)
    last_page = document.pages[-1].page if document.pages else 1
    chunks: list[dict[str, Any]] = []
    position = 0
    while position < len(full):
        raw_text = full[position:position + size]
        text = raw_text.strip()
        if text:
            # Tính trang bắt đầu/kết thúc theo vị trí offset trong full_text
            start_page = _page_at(position, page_map, last_page)
            end_offset = min(position + size - 1, len(full) - 1)
            end_page = _page_at(end_offset, page_map, last_page)
...
def semantic_chunks(document: DocumentText, size: int) -> list[dict[str, Any]]:
    """Ngắt theo ranh giới đoạn văn/câu; ưu tiên \\n\\n rồi đến chữ hoa đầu dòng.

    Tính page_start/page_end trực tiếp từ biến page.page — chính xác vì ta duyệt từng trang.
    """
    chunks: list[dict[str, Any]] = []
    buffer, first_page, last_page = "", 1, 1
    for page in document.pages:
        # Tách đơn vị ngữ nghĩa: đoạn (\n\n) hoặc dòng bắt đầu bằng chữ hoa/Đ
        units = [unit.strip() for unit in re.split(r"\n\s*\n|\n(?=[A-ZĐÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚÝĂĐƠƯ])", page.text) if unit.strip()]
        for unit in units:
            ...
            if len(unit) > size:
                # Đơn vị quá dài → tách thêm theo câu rồi theo khoảng trắng
...
async def process_pdf(pdf: Path, write: bool, size: int, overlap: int) -> dict[str, Any]:
    """Xử lý 1 file PDF: trích xuất → (OCR nếu cần) → 3 chiến lược chunking → báo cáo.

    Dry-run (write=False): luôn chạy chunking và in thống kê, KHÔNG ghi file và KHÔNG gọi
    LlamaParse — giúp người học xem kết quả mà không phát sinh chi phí API.
    Write (write=True): ghi raw/chunks/reports, gọi LlamaParse khi PDF cần OCR.
    """
    ...
    if bad_pages:
        if write:
            # Chỉ render ảnh và gọi OCR khi có cờ --write (tránh gọi API không cần thiết)
            ...
            # Dry-run: ghi chú sẽ cần OCR, tiếp tục chunking bằng text layer hiện có
            document.warnings.append(
                f"DRY-RUN: {len(bad_pages)} trang có dấu hiệu lỗi text layer "
                f"(trang {bad_pages}); sẽ gọi LlamaParse OCR khi chạy --write."
            )

    # Luôn chạy 3 chiến lược — kể cả dry-run — để in thống kê cho người học so sánh
    ...
    report: dict[str, Any] = {
        ...
        "statistics": {
            "fixed-size": statistics(fixed),
            "semantic": statistics(semantic),
            "hierarchical": statistics(hierarchical),
        },
        "warnings": document.warnings,
    }
    ...
    if write:
        ...
        for strategy_name, chunk_list in (
            ...
            write_json(OUTPUT_DIR / "chunks" / f"{stem}__{strategy_name}.json", chunk_list)
        write_json(OUTPUT_DIR / "reports" / f"{stem}__report.json", report)

    return report


def print_error_scenarios() -> None:
    """In báo cáo 3 tình huống lỗi đã được xử lý trong pipeline."""
    sep = "=" * 72
    print(f"\n{sep}")
    print("  BÁO CÁO 3 TÌNH HUỐNG LỖI ĐÃ XỬ LÝ TRONG PIPELINE (BUỔI 5)")
    print(sep)
    print("""
1. [Lỗi font / encoding / ký tự lạ trong text layer PDF]
   Dấu hiệu : ký tự '\\ufffd', dấu hiệu encoding sai (Ã., Â.), số lẫn trong từ
              (th6ng, di6m), hoặc trang rỗng hoàn toàn.
   Xử lý   : assess → đánh dấu trang bad; render toàn bộ file thành ảnh (không
              sửa PDF gốc) → gọi LlamaParse agentic OCR toàn file → chuẩn hoá NFC.
   Dry-run  : ghi chú trang bị lỗi vào warnings, vẫn chunking text layer hiện có.

2. [Văn bản không có cấu trúc phân cấp Chương / Điều]
   Dấu hiệu : Hierarchical chunker không khớp bất kỳ pattern CHƯƠNG/ĐIỀU/MỤC nào.
   Xử lý   : KHÔNG bịa heading. Ghi cảnh báo 'Không tìm thấy heading...' vào
              trường warnings của report. Vẫn sinh chunk bằng logic nội dung thuần.
   Ràng buộc: Không thêm cấu trúc giả vào metadata 'structure'.

3. [Chưa cấu hình / API Key Llama Cloud không hợp lệ]
   Dấu hiệu : RuntimeError khi gọi ocr_entire_file() do LLAMA_CLOUD_API_KEY trống.
   Xử lý   : Lỗi được bắt tại process_pdf() → in thông báo LỖI rõ ràng →
              trả về exit code 1 mà không làm sập toàn bộ batch. Các file PDF
              khác trong datademo/ vẫn tiếp tục được xử lý.
   Lưu ý   : API key chỉ được nạp qua os.getenv(); giá trị không bao giờ in ra log.
""")
    print(sep)
...
async def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="PDF → raw NFC → 3 chiến lược chunking (không embedding, không LLM)."
    )
    parser.add_argument(
        "--write", action="store_true",
        help="Ghi raw/chunks/reports ra output/ và gọi LlamaParse OCR nếu cần."
    )
    parser.add_argument("--file", type=Path, help="Chỉ xử lý một PDF cụ thể trong datademo/.")
    parser.add_argument("--chunk-size", type=int, default=1200,
                        help="Số ký tự tối đa mỗi chunk (mặc định 1200).")
    parser.add_argument("--overlap", type=int, default=150,
                        help="Số ký tự gối đầu giữa các chunk fixed-size (mặc định 150).")
    parser.add_argument("--no-error-report", action="store_true",
                        help="Bỏ qua in báo cáo tình huống lỗi ở cuối.")
    ...
    mode_label = "--write (ghi file + OCR nếu cần)" if args.write else "dry-run (chỉ thống kê, không ghi, không OCR)"
    print(f"\nChế độ: {mode_label}")
    print(f"Tham số: chunk-size={args.chunk_size}, overlap={args.overlap}")
    print(f"Số file PDF: {len(pdfs)}\n")
    ...
    for pdf in pdfs:
        ...
        try:
            ...
            print(f"LỖI khi xử lý {pdf.name}: {exc}")
            exit_code = 1  # Báo lỗi nhưng tiếp tục xử lý file tiếp theo
